...
let benchmark_obj = {
  ...
  secure: false,
  port: environment == "docker" ? 5000 : 3000,
...
function arrayToCSV(array, filename) {
  ...
  ensureDirectoryExists(`result/second_test/ws_${benchmark_obj.connection_interval}`);
...
connection_manager.createConnections().then(() => {
  ...
  connection_manager.sendRequests().then(() => {
    ...
    const filename = `result/second_test/ws_${benchmark_obj.connection_interval}/ws_${benchmark_obj.connection_interval}_raw_response_time_output_${Date.now()}.csv`
